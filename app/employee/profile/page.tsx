  ...
  return (
    <div className="animate-in fade-in duration-300 pb-12 space-y-8 max-w-5xl mx-auto ease-cg">
      ...
      <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row gap-8 items-start md:items-center relative overflow-hidden">
        ...
        <div className="relative z-10 bg-brand-50 border border-brand-200 p-5 rounded-2xl w-full md:w-auto shrink-0 flex items-center gap-4 shadow-sm">
          <div className="p-3 bg-white rounded-xl shadow-sm text-brand-600">
            <Activity className="w-6 h-6" />
          </div>
          <div>
            <p className="text-sm font-semibold text-brand-800 uppercase tracking-wider mb-0.5">Current Duty</p>
            <p className="text-ui-text-primary font-bold">{user.title.includes('Lead') ? 'On-Call Lead' : 'Active Shift'}</p>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-slate-100 flex items-center gap-3 justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg text-ui-text-primary">Recent Records</h2>
            </div>
              ...
              <Link key={sub.id} href={`/employee/submissions/${sub.id}`} className="block p-4 border border-slate-200 rounded-xl hover:border-brand-300 hover:shadow-sm transition-all group">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Submitted: {formatDate(sub.createdAt)}</p>
                  </div>
