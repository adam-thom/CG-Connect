...
export default function AdminSchedule() {
  ...
  const upcomingVacancies: { date: string, missingRoles: string[] }[] = [];
  ...
  for (let i = 0; i < 14; i++) {
    ...
    const filledRoles = dayEntries.map(e => e.roleType);
    const missingRoles = REQUIRED_ROLES.filter(role => !filledRoles.includes(role));
    
    if (missingRoles.length > 0) {
      upcomingVacancies.push({ date: dateStr, missingRoles });
      if (upcomingVacancies.length >= MAX_VACANCIES) break;
    }
  }
      ...
      <div className="flex flex-col xl:flex-row gap-8 items-start relative z-0">
        ...
        <div className="w-full xl:w-80 space-y-6 shrink-0 z-0">
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm relative overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-red-50 rounded-bl-full -mr-4 -mt-4 pointer-events-none"></div>
            <h3 className="font-bold text-red-900 mb-4 relative z-10">Upcoming Vacancies</h3>
            <div className="space-y-3 relative z-10">
              {upcomingVacancies.length === 0 ? (
                 <p className="text-sm font-medium text-emerald-700">No vacancies in the next 14 days. Staffing is full.</p>
              ) : (
                 upcomingVacancies.map((v, i) => {
                   const displayDate = new Date(v.date + "T12:00:00").toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
                   return (
                     <div key={i} className="text-sm border-l-2 border-red-300 pl-3">
                       <p className="font-semibold text-red-800">{displayDate}</p>
                       <p className="text-xs text-red-700 font-medium mt-0.5 max-w-full truncate" title={v.missingRoles.join(', ')}>
                         Missing: {v.missingRoles.join(', ')}
                       </p>
                     </div>
                   );
                 })
